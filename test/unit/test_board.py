...
def test_regenerate_apples_with_step():
    ...
    for i in range(15):
        actions = {'Agent0': 1 if i % 2 == 0 else 3}
        for j in range(20):
            env.step(actions)
        env.step({'Agent0': 2})
    board_middle = deepcopy(env.board)
