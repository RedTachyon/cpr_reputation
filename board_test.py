...
import numpy as np
from board import HarvestGame, create_board, get_neighbors, regenerate_apples, random_board, NOOP, SHOOT
...
def test_create():
    ...
    all_apples = [get_neighbors(apple, (50, 50)) for apple in apples]
...
def test_neighbors():
    ...
    neighbors = get_neighbors((0, 0), (7, 7))
    expected = [
        (0, 0),
        (1, 0),
        (0, 1)
...
def test_obs():
    ...
    env._move_agent("Agent0", (50, 50))
    for direction in range(4):
        env._set_rotation("Agent0", direction)
        assert env.get_agent_obs("Agent0").shape == (20, 21)  # FIXME: this is currently failing
